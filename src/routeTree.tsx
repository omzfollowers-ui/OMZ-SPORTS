...
const indexRoute = createRoute({
  ...
  component: () => {
    ...
    const feed = allPosts.filter((p) => p.slug !== lead?.slug).slice(0, 12);
              ...
              <Link to="/posts/$slug" params={{ slug: lead.slug }}
                ...
                <div className="p-6 md:p-8">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="kicker text-[0.62rem]" style={{ color: "var(--flame)" }}>Lead story</span>
                    <span className="kicker text-[0.62rem]" style={{ color: "var(--muted)" }}>·</span>
                    <span className="kicker text-[0.62rem]" style={{ color: "var(--muted)" }}>{lead.categories[0]}</span>
                  </div>
                  <h2 className="display text-4xl md:text-5xl mt-3 group-hover:text-[var(--flame)] transition-colors"
                    style={{ color: "var(--paper)" }}>{lead.title}</h2>
                  <p className="mt-4 font-serif text-lg" style={{ color: "var(--muted)" }}>{lead.summary}</p>
                ...
                {sidebar.map((post, i) => (
                  <li key={post.slug}>
                    <Link to="/posts/$slug" params={{ slug: post.slug }}
                      ...
                      <span className="display text-3xl w-8 shrink-0" style={{ color: "var(--line)" }}>{i + 1}</span>
                      <div>
                        <span className="kicker text-[0.58rem]" style={{ color: "var(--flame)" }}>{post.categories[0]}</span>
                        <h3 className="font-serif text-base mt-1 group-hover:text-[var(--flame)] transition-colors"
                          style={{ color: "var(--paper)" }}>{post.title}</h3>
                      </div>
                    </Link>
                  </li>
                ))}
        ...
        <section className="mx-auto max-w-6xl px-4 py-12">
          <div className="rounded-2xl border p-8 md:p-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6"
            ...
            <div>
              ...
              <h2 className="display text-3xl md:text-4xl" style={{ color: "var(--paper)" }}>Reach 4.7M sports fans every month</h2>
          ...
          <div className="flex items-center gap-4 mb-8 border-b pb-4" style={{ borderColor: "var(--line)" }}>
            ...
            {["Football", "Basketball", "Tennis", "Transfers"].map((cat) => (
              <Link key={cat} to="/category/$category" params={{ category: cat }}
                className="kicker rounded-full px-3 py-1.5 text-[0.62rem] transition-colors border"
                style={{ color: "var(--muted)", borderColor: "var(--line)" }}>{cat}</Link>
            ))}
          </div>
...
const categoryRoute = createRoute({
  ...
  component: () => {
    ...
    const filtered = allPosts.filter((p) => p.categories.includes(category));
    return (
    ...
    if (!post) {
      return (
        <div className="mx-auto max-w-6xl px-4 py-32 text-center">
          ...
          <Link to="/" className="kicker mt-8 inline-block rounded-full px-6 py-3 text-xs text-black"
            style={{ backgroundColor: "var(--flame)" }}>Back to the feed</Link>
        </div>
      );
    }
      ...
      <div>
        <article className="mx-auto max-w-3xl px-4 py-12">
          <Link to="/category/$category" params={{ category: post.categories[0] }}
            className="kicker text-[0.62rem] mb-6 inline-block" style={{ color: "var(--muted)" }}>
            ← {post.categories[0]}
          </Link>
          {post.sponsored && (
            <span className="kicker text-[0.58rem] mb-4 rounded-full inline-block px-3 py-1 ml-3"
              style={{ color: "var(--ink)", backgroundColor: "var(--gold)" }}>Sponsored</span>
          )}
          <h1 className="display text-4xl md:text-6xl mt-4 rise" style={{ color: "var(--paper)" }}>{post.title}</h1>
          <p className="font-serif text-xl mt-6" style={{ color: "var(--muted)" }}>{post.summary}</p>
          <div className="flex items-center gap-3 mt-6 pt-6 border-t" style={{ borderColor: "var(--line)" }}>
            <span className="display text-xl w-8 h-8 rounded-full flex items-center justify-center text-sm"
              style={{ backgroundColor: "var(--ink-3)", color: "var(--paper)" }}>{post.author[0]}</span>
            <div>
              <p className="text-sm font-medium" style={{ color: "var(--paper)" }}>{post.author}</p>
              <p className="kicker text-[0.58rem]" style={{ color: "var(--muted)" }}>{post.date} · {readTime} min read</p>
            </div>
          </div>
          <div className="article-body mt-10" dangerouslySetInnerHTML={{ __html: html }} />
          <div className="mt-12 rounded-2xl border p-8" style={{ borderColor: "var(--line)", backgroundColor: "var(--ink-2)" }}>
            ...
            <Link to="/advertise" className="kicker mt-4 inline-block text-[0.62rem]" style={{ color: "var(--flame)" }}>
        ...
        {related.length > 0 && (
          <section className="mx-auto max-w-6xl px-4 pb-16">
            <div className="border-t pt-10" style={{ borderColor: "var(--line)" }}>
              <p className="kicker text-[0.62rem] mb-6" style={{ color: "var(--muted)" }}>More from {post.categories[0]}</p>
